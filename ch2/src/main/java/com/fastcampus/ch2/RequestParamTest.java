...
public class RequestParamTest {
	@ExceptionHandler(Exception.class)//괄호안에 있는 예외가 발생했을때 아래 메서드 호출
	public String catcher(Exception ex) {
		return "yoilError";
	}
	@RequestMapping("/requestParam")
	public String main(HttpServletRequest request) {
		String year = request.getParameter("year"); // yea= 출력
...
//		http://localhost/ch2/requestParam7?year   ---->> 400 Bad Request, nested exception is java.lang.NumberFormatException: For input string: "" 
	...
	public String main8(@RequestParam(required=false, defaultValue ="2021") int year) {   //required = false(필수입력x) 일떈 defaultValue필수 없으면 500 서버오류남
	...
	//	http://localhost/ch2/requestParam8?year   ---->> 400 Bad Request, nested exception is java.lang.NumberFormatException: For input string: "" 
		System.out.printf("[%s]year=[%s]%n", new Date(), year); //매개변수 int 변환이 필요할떈 예외처리를 해서 사용자 친화적인화면 출력 
		return "yoil";
	}
	
	@RequestMapping("/requestParam9") 
	public String main9(@RequestParam(required=true) int year) {  //required true 필수입력일떈 예외처리해서 해줘야해서 사용자가 올바른값을 입력할수록 유도 
